...
public class Message {
    ...
    @Nonnull public static final ChatComponent NOT_A_PLAYER_EN = new ChatComponent(new LanguageKey(Language.ENGLISH, MessageKey.NOT_A_PLAYER), "§4%prefix%§4 %player%§c is not a player");
    ...
    @Nonnull public static final ChatComponent NOT_A_PLAYER_DE = new ChatComponent(new LanguageKey(Language.GERMAN, MessageKey.NOT_A_PLAYER), "§4%prefix%§4 %player%§c ist kein spieler");

    static {
        for (Language language : Language.values()) {
            JsonConfig jsonConfig = new JsonConfig(language.getFile());
            JsonElement jsonElement = jsonConfig.getJsonElement();
            ...
            if (jsonElement.isJsonObject()) {
                ...
                for (ChatComponent component : ChatComponent.getMessages()) {
                    if (component.getLanguageKey().getLanguage().equals(language)) {
                        if (jsonObject.has(component.getLanguageKey().getMessageKey().getKey())) {
                            String value = jsonObject.get(component.getLanguageKey().getMessageKey().getKey()).getAsString();
                            component.setText(value);
                            Logger.debug.println("§aLoaded component §8'§6" + component.getLanguageKey().getMessageKey().getKey() + "§8'§a with value §8'§6" + value + "§8'§6 for language §8'§r" + component.getLanguageKey().getLanguage().getName() + "§8'");
                        } else {
                            if (component.getLanguageKey().getLanguage().equals(language)) {
                                jsonObject.addProperty(component.getLanguageKey().getMessageKey().getKey(), component.getText());
                                Logger.debug.println("§aAdded component §8'§6" + component.getLanguageKey().getMessageKey().getKey() + "§8'§a with value §8'§6" + component.getText() + "§8'§6 for language §8'§r" + component.getLanguageKey().getLanguage().getName() + "§8'");
                            }
                        }
                    }
                }
