package net.nonswag.tnl.listener.v1_14_R1.api.playerAPI;
...
public class TNLPlayer {
    ...
    private ScoreboardTeam tnlOptionPacketTeam = tnlOptionPacketScoreboard.getTeam("TNLOptionPacket");

    @Deprecated
    public TNLPlayer(Player player) {
        this.player = player;
        if (getTnlOptionPacketTeam() == null) {
            setTnlOptionPacketTeam(new ScoreboardTeam(tnlOptionPacketScoreboard, "TNLOptionPacket"));
        }
    }
    ...
    public void setTnlOptionPacketTeam(ScoreboardTeam tnlOptionPacketTeam) {
        this.tnlOptionPacketTeam = tnlOptionPacketTeam;
    }
    ...
    public void disconnect(String kickMessage) {
        NMSMain.runTask(() -> {
            if (!((CraftPlayer) player).getHandle().playerConnection.processedDisconnect) {
    ...
    public String getNamedWorld() {
        if (getNamedWorldHashMap() != null && getNamedWorldHashMap().get(player.getWorld()) != null) {
        ...
        try {
            if (!file.exists()) {
                if (!file.createNewFile()) {
                    NMSMain.stacktrace("Failed to create save file");
                }
            }
            if (file.exists()) {
